...
export async function POST(req) {
    try {
        ...
        const { error } = await supabase
            ...
            .eq('id', 1); // Assuming the main config row has ID 1

        if (error) {
            console.error('Supabase error updating config:', error);
            return NextResponse.json({ error: 'Failed to update config in database' }, { status: 500 });
        }

        return NextResponse.json({ message: 'Config updated successfully in database' });
    } catch (error) {
        return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }
}
